...
$$\{ 1, 2, 3 \} \rightarrow \{ 1, 2, 4 \}$$
$$\{ 1, 2, 3 \} \rightarrow \{ 1, 3, 4 \}$$
$$\{ 1, 2, 3 \} \rightarrow \{ 2, 3, 4 \}$$
and each has 6 permutations, totaling 24 injective maps. None of them are surjective, hence none is a bijection.

\noindent
(b)
Following the same procedure, there are $\binom{8}{10}$ choices on the right side, and each has $8!$ permutations.

\subsection*{Exercise 2}
If $A$ is finite, then any subset of $A$ is finite, by corollary 6.6. The contrapositive solves the exercise.

\subsection*{Exercise 3}
$$(0, 0, 0, \ldots) \rightarrow (1, 0, 0, \ldots)$$
$$(1, 0, 0, \ldots) \rightarrow (0, 1, 0, \ldots)$$
$$(0, 1, 0, \ldots) \rightarrow (0, 0, 1, \ldots)$$
$$\vdots$$
Basically, every binary number is mapped to a one-hot encoding. Left side is the entire $X^\omega$, while the right side is a proper subset.

\subsection*{Exercises 4-7}
Too lazy.
...
\subsection*{Exercise 1}
There are 8 subsets to check for openness. Of course, the empty set and $A$ are already open. $\{ a \}$ is open because $p^{-1}(\{ a \}) = (0, \inf)$ is open. $\{ b \}$ is open because $p^{-1}(\{ b \}) = (-\inf, 0)$ is open. $\{ c \}$ is not open, because its preimage is a point, which is closed. Since $\{ a \}, \{ b \}$ are open, then their union is open. $\{ b, c \}$ is not open, because their preimage $[0, \inf)$ is half-open. Likewise, $\{ a, c \}$ is not open.
