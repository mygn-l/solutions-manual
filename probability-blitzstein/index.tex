...
(a) If he chooses 1 stat courses, he chooses 6 of the 15 non-stat courses. If he chooses 2 stat courses, he chooses 5 of the 15 non-stat courses. And so on. He stops at 5 stat courses.
$$\binom{5}{1}\binom{15}{6} + \binom{5}{2}\binom{15}{5} + \binom{5}{3}\binom{15}{4} + \binom{5}{4}\binom{15}{3} + \binom{5}{5}\binom{15}{2}$$

\subsubsection*{Approach 2}
(a) Finding all combinations with at least 1 statistics course is equivalent to the complement of having 0 statistics course. The total amount of combinations can be written as $\binom{20}{7}$. The amount of combinations without statistics courses is $\binom{15}{7}$.
...
\noindent
(c) A card can only be included in a one hand. Once 13 cards are picked, they are not replaced in the pack.
...
$$\frac{44\binom{13}{2}\binom{4}{2}^2}{\binom{52}{5}}$$
...
\subsection*{Exercise 4}
(a)
We normalize with respect to the tail mass.
$$P(X \leq x \mid X > c) = \frac{F(x) - F(c)}{1 - F(c)}$$

\noindent
(b)
The derivative is:
$$P(x \mid X > c) = \frac{f(x)}{1 - F(c)}$$

\noindent
(c)
$f(x)$ is positive, because it's a PDF. Also, $1 \geq F(c)$ since CDF approaches 1 from below. Hence, the entire thing is positive. Next, we derive
$$\int_c^\infty \frac{f(x)}{1 - F(c)}$$
$$= \frac{1}{1 - F(c)} \int_c^\infty f(x)$$
$$= \frac{1}{1 - F(c)} (F(\infty) - F(c))$$
$$= \frac{1}{1 - F(c)} (1 - F(c)) = 1$$

\subsection*{Exercise 5}
(a)
$$E[A] = \int_0^1 \pi r^2 dr = \pi \left[ \frac{1}{3} r^3 \right]_0^1 = \frac{\pi}{3}$$
$$E[(A - E[A])^2] = E[A^2] - E[A]^2$$
$$= \int_0^1 (\pi r^2)^2 dr - \left(\frac{\pi}{3}\right)^2$$
$$= \pi^2 \left[ \frac{1}{5} r^5 \right]_0^1 - \frac{\pi^2}{9} = \frac{\pi^2}{5} - \frac{\pi^2}{9}$$


\noindent
(b)
$$F_A(x) = P(A \leq x) = P(\pi r^2 \leq x) = P \left( r \leq \sqrt{\frac{x}{\pi}} \right) = F_R \left(\sqrt{\frac{x}{\pi}} \right) = \sqrt{\frac{x}{\pi}}$$
$$f_A(x) = \frac{1}{2\sqrt{\pi x}}$$

\subsection*{Exercise 6}
(a)
$$\sigma = \sqrt{\int_0^1 (x - 0.5)^2 dx} = \sqrt{\left[ \frac{1}{3}(x - 0.5)^3 \right]_0^1} = \sqrt{\frac{2}{3} \cdot 0.5^3} = \frac{1}{2\sqrt{3}} \approx 29\%$$
hence $2 \cdot 29\% = 58\%$ is inside 1 standard deviation. And $100\%$ is within 2+ standard deviation.

\noindent
(b)
$\sigma = \frac{1}{\lambda} = 1$. Also, $\mu = \frac{1}{\lambda} = 1$ as well. Hence, inside 1 standard deviation, we have
$$\int_0^2 e^{-x} dx = \left[ -e^{-x} \right]_0^2 = -\frac{1}{e^2} + 1 \approx 84\%$$
Within 2, we have
$$\left[ -e^{-x} \right]_0^3 = -\frac{1}{e^3} + 1 \approx 95\%$$
and within 3, we have
$$\left[ -e^{-x} \right]_0^4 = -\frac{1}{e^4} + 1 \approx 98\%$$

\noindent
(c)
$\sigma = \mu = 2$. Hence, inside 1 standard deviation, we have
$$\int_0^4 \frac{1}{2} e^{-\frac{x}{2}} dx = \left[ -e^{-\frac{x}{2}} \right]_0^4 = -\frac{1}{e^2} + 1 \approx 84\%$$
then within 2
$$\left[ -e^{-\frac{x}{2}} \right]_0^6 = -\frac{1}{e^3} + 1 \approx 95\%$$
and within 3
$$\left[ -e^{-\frac{x}{2}} \right]_0^8 = -\frac{1}{e^4} + 1 \approx 98\%$$
Thus, there is a similar rule for exponential.
